const getFromLocalStorage = () => {
  const storedBookmarksIdsStringified = localStorage.getItem("bookmarksIds");
  if (storedBookmarksIdsStringified) {
    const storedBookmarks = JSON.parse(storedBookmarksIdsStringified);
...
const saveToLocalStorage = (newStoredBookmarksIds) => {
  const newStoredBookmarksIdsstringified = JSON.stringify(
    newStoredBookmarksIds
  );
  localStorage.setItem("bookmarksIds", newStoredBookmarksIdsstringified);
};
const removeFromLocalStorage = (id) => {
  const storedBookmarksIds = getFromLocalStorage();
  const remainingBookmarkIds = storedBookmarksIds.filter(
    (storedBookmarksId) => storedBookmarksId !== id
  );
...
const removeMarkReadsIdFromLocalStorage = (blog) => {
  const storedMarkReadsIds = getMarkReadsIdsFromLocalStorage();
  const remainingStoredMarkReadsIds = storedMarkReadsIds.filter(
    (readId) => readId !== blog.id
  );
